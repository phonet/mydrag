;(function( $ ){
	 ...
	 *        target[string]: 		可选，jQuery事件委托选择器字符串，默认'tr'
		...
		return this.each(function(){
			...
			that.on('mousedown.DDSort',settings.target,function(e){
				...
				$doc.on('mousemove.DDSort',function(e){
                    ...
                    // 鼠标拖动的范围(纵向)
					if(top<that.offset().top || top > that.offset().top+that.outerHeight()-thisOuterHeight){
						return;
					}
					/*if(left<that.offset().left || left > that.offset().left+that.width()){
						return;
					}*/
				...
				.on('mouseup.DDSort', function(){
					...
					if(!hasClone ){
						...
						if(settings.up && typeof(settings.up) == 'function'){
                            //执行拖拽后的回调函数
							settings.up(THIS);
						}
					}
			...
			.on('mouseout.DDSort',settings.target,function(e){
				return false;
		});
		});
	};
